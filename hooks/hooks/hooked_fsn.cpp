...
void weather()
{
	...
	while (client_class)
	{
		if (client_class->m_ClassID == IdManager->ClassIdMap[FNV1A::HashConst("CPrecipitation")])
			break;
...
{
	static auto original_fn = g_ctx.test_hooks ? oFrameStageNotify_t : client_hook->get_func_address <FrameStageNotify_t>(37);
	...
	if (g_ctx.globals.updating_skins && m_clientstate()->m_delta_tick > 0) //-V807
	...
	if (g_ctx.local() && g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_RECOIL] && g_ctx.local()->is_alive() && aim_punch && view_punch)
	{

...
void __fastcall hooks::hooked_postscreeneffects(void* thisptr, void* edx, CViewSetup* setup)
{
	...
	static auto original_fn = g_ctx.test_hooks ? oDoPostScreenEffects_t : clientmode_hook->get_func_address <DoPostScreenEffects_t>(44);
	...
	for (auto i = 0; i < m_glow()->m_GlowObjectDefinitions.Count(); i++) //-V807
	{
		...
		else if (g_cfg.esp.weapon[WEAPON_GLOW] && (client_class->m_ClassID == IdManager->ClassIdMap[FNV1A::HashConst("CAK47")] || client_class->m_ClassID == IdManager->ClassIdMap[FNV1A::HashConst("CDEagle")] || client_class->m_ClassID >= IdManager->ClassIdMap[FNV1A::HashConst("CWeaponAug")] && client_class->m_ClassID <= IdManager->ClassIdMap[FNV1A::HashConst("CWeaponZoneRepulsor")])) //-V648
		{
			float color[4] =
			{

...
{
	if (!this || m_clientstate()->m_delta_tick <= 0) //-V704
		return false;
	...
	return client_class->m_ClassID == IdManager->ClassIdMap[FNV1A::HashConst("CCSPlayer")];
}
...
{
	return get< c_baseplayeranimationstate* >(g_csgo.PlayerAnimState);
...
CStudioHdr* player_t::m_studioHdr() {
	return get< CStudioHdr* >(g_csgo.studioHdr);
}


 CStudioHdr* player_t::GetModelPtr() {
 ...
 AnimationLayer* player_t::m_AnimOverlay() {
	 return get< AnimationLayer* >(g_csgo.AnimOverlay);
 }
 void player_t::SetAbsAngles(const Vector& angles) {
	 using SetAbsAngles_t = void(__thiscall*)(decltype(this), const Vector&);
 ...
 void player_t::BuildTransformations(CStudioHdr* hdr, Vector* pos, quaternion_t* q, const matrix3x4_t& transform, int mask, uint8_t* computed) {
	 using BuildTransformations_t = void(__thiscall*)(decltype(this), CStudioHdr*, Vector*, quaternion_t*, matrix3x4_t const&, int, uint8_t*);
	 return get_method< BuildTransformations_t >(this, 189)(this, hdr, pos, q, transform, mask, computed);
 }

 void player_t::StandardBlendingRules(CStudioHdr* hdr, Vector* pos, quaternion_t* q, float time, int mask) {
	 using StandardBlendingRules_t = void(__thiscall*)(decltype(this), CStudioHdr*, Vector*, quaternion_t*, float, int);

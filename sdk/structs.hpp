...
public:
    ...
    float* m_flPoseParameters() {
        return (float*)((uintptr_t)this + netvars::get().get_offset(crypt_str("CCSPlayer"), crypt_str("m_flPoseParameter")));
    }
    AnimationLayer* m_AnimOverlay();
	void SetAbsAngles(const Vector& angles);
	void SetAbsOrigin(const Vector& origin);
    void SetAnimLayers(AnimationLayer* layers) {
        std::memcpy(m_AnimOverlay(), layers, sizeof(AnimationLayer) * 13);
    }
    void AddEffect(int effects) {
        m_fEffects() |= effects;
    }
    void GetAnimLayers(AnimationLayer* layers) {
        std::memcpy(layers, m_AnimOverlay(), sizeof(AnimationLayer) * 13);
    }

    void SetPoseParameters(float* poses) {
        std::memcpy(m_flPoseParameters(), poses, sizeof(float) * 24);
    }
    
 
    void GetPoseParameters(float* poses) {
        std::memcpy(poses, m_flPoseParameters(), sizeof(float) * 24);
    }
	...
	void BuildTransformations(CStudioHdr* hdr, Vector* pos, quaternion_t* q, const matrix3x4_t& transform, int mask, uint8_t* computed);
	void StandardBlendingRules(CStudioHdr* hdr, Vector* pos, quaternion_t* q, float time, int mask);
	uint32_t& m_fEffects();
    uint32_t& m_iEFlags();

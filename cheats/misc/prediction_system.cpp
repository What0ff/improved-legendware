...
{
	auto data = &netvars_data[m_clientstate()->m_command_ack % MULTIPLAYER_BACKUP]; //-V807
...
{
	auto data = &netvars_data[(m_clientstate()->m_command_ack - 1) % MULTIPLAYER_BACKUP]; //-V807
...
void engineprediction::predict(CUserCmd* m_pcmd)
{
	...
	if (m_clientstate()->m_delta_tick > 0)  //-V807

...
void Game_Msg(const char* msg, ...)
{
	...
	m_cvar()->ConsoleColorPrintf(Color(255, 0, 0), "[legendware] ");
...
void Features::CEventLogger::Draw_D3D9()
{
	...
	for (auto& l : logs)
	{
		render::get().text(fonts[FONT_SHONAX], 5, 5 + offset + l.yOffset, Color(117, 187, 253), HFONT_CENTERED_NONE, "[legendware] ");

...
bool draw_config_button(const char* label, const char* label_id, bool load, bool save, int curr_config, bool create = false)
{
	bool pressed = false;
	ImGui::SetCursorPosX(8);
	if (ImGui::PlusButton(label, 0, ImVec2(274, 26), label_id, ImColor(vars.misc.menu_color.r() / 255.f, (vars.misc.menu_color.g()) / 255.f, (vars.misc.menu_color.b()) / 255.f), true))vars.selected_config = curr_config;
	...
	if (ImGui::BeginPopup(label_id, ImGuiWindowFlags_NoMove))
	{
		...
		if (!create) {
			if (LabelClick(crypt_str("Load"), &one, label_id))
			{
				load_config();
			}
			if (LabelClick(crypt_str("Save"), &two, label_id))
			{
				save_config();
			}
		}
...
#define VERSION crypt_str("priority.cc")
void c_menu::waterwark()
{
	...
	if (vars.misc.watermarkadditives[WATERMARK_TIME])
		watermark += crypt_str(" | ") + csgo.globals.time;
...
void c_menu::draw(bool is_open)
{
		...
		uname = comp_name(); build = "[alpha]"; build_at = __DATE__; build_at += " "; build_at += __TIME__; sponsor = "";
	...
	ImGui::Begin("##GUI", NULL, ImGuiWindowFlags_::ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_::ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_::ImGuiWindowFlags_NoBackground);
	{
		...
		{//draw
			...
			ImGui::PushFont(font);
			draw->AddText(ImVec2(p.x + 5, p.y + s.y - 20 + (10 - ImGui::CalcTextSize(std::string("prio alpha " + build + " | build at: " + build_at + " | " + sponsor).c_str()).y / 2)), ImColor(220, 220, 220, int(public_alpha * 255)), std::string("prio alpha " + build + " | build at: " + build_at + " | " + sponsor).c_str());
			...
			ImGui::PopFont();
			{
				...
				if (tab_anim > 220 && ss < 1.f)
					...
					ss -= 0.04f;
				ImGui::PushFont(name);
				ImGui::GetForegroundDrawList()->AddText(ImVec2(p.x + tab_anim / 2 - ImGui::CalcTextSize("prio").x / 2, p.y + 20 - ImGui::CalcTextSize("prio").y / 2), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, ImClamp(ss * 2.f, 0.f, 1.f)), "prio");
				ImGui::PopFont();
			}
		}
		...
		ImGui::BeginGroup(/*MAIN SPACE START*/);
		{
			{//tabs
				{
					...
					if (ImGui::tab("S", "Lua", "Cheat modify by players", tab == 6, tab_anim) && last_tab == tab && !preview_reverse) tab = 6; if (ImGui::IsItemHovered())tab_hovered = true;
					ImGui::SetCursorPosY(390);
					if (ImGui::tab("C", "Configs", "Save, Load, Share configs", tab == 7, tab_anim) && last_tab == tab && !preview_reverse) tab = 7; if (ImGui::IsItemHovered())tab_hovered = true;
					ImGui::PopFont();
				}
			...
			ImGui::PushFont(c_menu::get().font);
			{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						...
						else//main
						{
							ImGui::MenuChild(crypt_str("Main"), ImVec2(290, 400 - 20 * stab_anim));
							{
								...
								ImGui::Checkbox(crypt_str("Debug Resolver"), &vars.ragebot.debug_resolver);
								ImGui::Checkbox(crypt_str("Delay Shot"), &vars.ragebot.delay_shot);
							}
							ImGui::EndMenuChild();
							...
							else
							{
								...
								if (vars.antiaim.type[type].desync)
								{
									...
									if (type != ANTIAIM_STAND || !vars.antiaim.lby_type)
									{
										...
										ImGui::SliderInt(crypt_str("Body lean"), &vars.antiaim.type[type].body_lean, 0, 60);
										ImGui::SliderInt(crypt_str("Inverted body lean"), &vars.antiaim.type[type].inverted_body_lean, 0, 60);
									}
						...
						if (stab2) {//players
							...
							ImGui::MenuChild(crypt_str("Chams"), ImVec2(290, 400 - 20 * stab_anim2));
							{
								...
								if (category == LOCAL)
								{
									...
									{
										draw_combo(crypt_str("Player model T"), vars.player.player_model_t, player_model_t, ARRAYSIZE(player_model_t));
										padding(0, 3);
										draw_combo(crypt_str("Player model CT"), vars.player.player_model_ct, player_model_ct, ARRAYSIZE(player_model_ct));
									}
								}
				...
				case 6://cefege pricupi eblan
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						ImGui::MenuChild("Lua", ImVec2(290, 420));
						{
							...
							if (ImGui::CustomButton(crypt_str("Refresh"), crypt_str("##LUA__REFRESH"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
							{
							...
							if (!scripts.empty()) {
								if (ImGui::CustomButton(crypt_str("Edit"), crypt_str("##LUA__EDIT"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
								{
									loaded_editing_script = false;
							...
							if (ImGui::CustomButton(crypt_str("Load"), crypt_str("##SCRIPTS__LOAD"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
							{
						...
						{
							ImGui::Checkbox(crypt_str("Developer mode"), &vars.scripts.developer_mode);
				...
				case 7://cfg
				{
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						ImGui::MenuChild("Configs", ImVec2(290, 420));
						{
							...
							{
								draw_config_button("Add new config", "", false, false, false, false);
								for (int i = 0; i < files.size(); i++)
								{
									bool load, save = false;
									draw_config_button(files.at(i).c_str(), std::string(files.at(i) + "").c_str(), load, save, i);
								}
							}
							ImGui::EndChild();
						}

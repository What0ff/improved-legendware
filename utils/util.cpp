...
namespace util
{
	...
	bool is_button_down(int code)
	{
		...
		//	return false;
		//if (m_inputsys()->IsButtonDown((ButtonCode_t)code))
		//	return true;
		//else
		//	return false;
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		do
		{
			...
			if (command->m_tickcount != INT_MAX && m_clientstate()->m_delta_tick > 0)
				m_prediction()->Update(m_clientstate()->m_delta_tick, true, m_clientstate()->m_last_command_ack, m_clientstate()->m_last_outgoing_command + m_clientstate()->m_choked_commands);

			command->m_command_number = sequence_number;
			command->m_predicted = command->m_tickcount != INT_MAX;

			++m_clientstate()->m_choked_commands; //-V807

			if (m_clientstate()->m_net_channel)
			{
				++m_clientstate()->m_net_channel->m_nChokedPackets;
				++m_clientstate()->m_net_channel->m_nOutSequenceNr;
			}

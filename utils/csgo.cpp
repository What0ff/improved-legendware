...
IBaseClientDLL * C_CSGO::m_client() {
	if (!p_client)
		if (!g_ctx.test_hooks)
			...
			p_client = Interface<IBaseClientDLL>(crypt_str("client.dll"), crypt_str("VClient018")); //oof
...
{
	if (!p_enginetool)
		p_enginetool = Interface< IEngineTool >("engine.dll", "VENGINETOOL003"); //newest from csgo sdk

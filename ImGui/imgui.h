...
// Call and read ui::ShowDemoWindow() in imgui_demo.cpp for demo code.
...
namespace ui
{
    IMGUI_API bool          ColorEdit4(const char* label, Color* col, int max, ImGuiColorEditFlags flags = 0);
    bool ColorButtonSkeet(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0));
	bool ColorEdit4Skeet(const char* label, Color* col, ImGuiColorEditFlags flags);
	bool ColorPicker4Skeet(const char* label, float col[4], ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);
    ...
    IMGUI_API bool          Checkbox(const char* label, bool* v, int max = 40);
    IMGUI_API bool          Checkbox_keybind(const char* label, bool* v);
    ...
    bool ListBoxConfigArray(const char* label, int* currIndex, std::vector<std::string>& values, int height);
	bool ListBoxConfigArraySkeet(const char* label, int* currIndex, std::vector<std::string>& values, int height);
	...
	bool SingleSelectOT(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), int items_count);
    bool SingleSelectSkeet(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), int items_count);
    IMGUI_API bool          MultiSelect(const char* label, std::vector<int> data, std::vector<const char*> items);
	IMGUI_API bool          MultiSelect(const char* label, std::unordered_map<int, bool>* data, std::vector<const char*> items);
    IMGUI_API bool          MultiCombo(const char* label, bool* item, const char* const* items, int items_count, int height_in_items = -1);
    IMGUI_API bool          MultiCombo(const char* label, bool* item, const char* items_separated_by_zeros, int height_in_items = -1);      // separate items with \0, end item-list with \0\0
    IMGUI_API bool          MultiCombo(const char* label, bool* item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items = -1);
    // Widgets: Drags
    ...
    IMGUI_API bool          SliderIntSkeet(const char* label, int* v, int v_min, int v_max, const char* format = "%d", int remove_from_fmt = 0);
    ...
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], int max, ImGuiColorEditFlags flags = 0);
   
    bool ColorEdit4Shonax(const char* label, Color* col1, ImGuiColorEditFlags flags, int max);
    ...
    bool BeginChildFrameSkeet(const char* label, const ImVec2& size, ImGuiWindowFlags extra_flags);
    bool ListBoxHeaderSkeet(const char* label, const ImVec2& size_arg = ImVec2(0, 0));
    IMGUI_API bool          ListBoxHeader(const char* label, int items_count, int height_in_items = -1); // "
	bool ListBoxHeaderSkeet(const char* label, int items_count, int height_in_items = -1);
    IMGUI_API void          ListBoxFooter();                                                    // terminate the scrolling region. only call ListBoxFooter() if ListBoxHeader() returned true!

    void ListBoxFooterSkeet();
    ...
    IMGUI_API void          SetTooltip(const char* fmt, ...) IM_FMTARGS(1);                     // set a text-only tooltip, typically use with ui::IsItemHovered(). overidde any previous call to SetTooltip().
	...
	bool BeginPopupExSkeet(ImGuiID id, ImGuiWindowFlags extra_flags);
	...
	bool BeginPopupSkeet(const char* str_id, ImGuiWindowFlags flags);
    ...
    IMGUI_API void          TabButton_icon(const char* label, const char* icon, int* selected, int num, int total);
    //IMGUI_API void          TabButton(const char* label, int* currenttab, int number, int total);
    ...
    IMGUI_API ImVec2        GetMousePos();                                                      // shortcut to ui::GetIO().MousePos provided by user, to be consistent with other calls
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_AnyWindow                     = 1 << 2,   // IsWindowFocused(): Return true if any window is focused. Important: If you are trying to tell how to dispatch your low-level inputs, do NOT use this. Use ui::GetIO().WantCaptureMouse instead.
...
enum ImGuiCol_
{
    ...
    ImGuiCol_Shonax_Skeet_v2_shadow,
    ImGuiCol_Shonax_Skeet_v2_hovered,
    ImGuiCol_Shonax_Skeet_v2_text,
    ImGuiCol_COUNT
...
// Enumateration for ui::SetWindow***(), SetNextWindow***(), SetNextTreeNode***() functions
...
// During the frame, use ui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style values,
// and ui::PushStyleColor(ImGuiCol_XXX)/PopStyleColor() for colors.
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    // You can obtain the ImDrawData* by calling ui::GetDrawData() after Render(). See example applications if you are unsure of how to implement this.
...
namespace ui
{
    ...
    static inline bool  IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0); return false; } // This was misleading and partly broken. You probably want to use the ui::GetIO().WantCaptureMouse flag instead.
...
// Usage: static ImGuiOnceUponAFrame oaf; if (oaf) ui::Text("This will be called only once per frame");
...
// Each ImGui window contains its own ImDrawList. You can use ui::GetWindowDrawList() to access the current window draw list and draw custom primitives.
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use ui::GetDrawListSharedData() to get the one from current ImGui context)
    ...
    // If you want to create ImDrawList instances, pass them ui::GetDrawListSharedData() or create and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)
...
struct ImFontAtlas
{
    ...
    bool                        Locked;             // Marked as Locked by ui::NewFrame() so attempt to modify the atlas will assert.

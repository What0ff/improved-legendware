...
namespace ui
{
	...
	bool CheckboxSkeet(const char* label, bool* v);

	bool SingleSelectSkeet(const char* label, int* current_item, std::vector<const char*> items);

	bool BeginComboSkeet(const char* label, const char* preview_value, ImGuiComboFlags flags, int items);

	bool SelectableOT(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg = ImVec2(0, 0));

	bool SelectableSkeet(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));

    bool SelectableSkeet(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));

    bool SliderScalarSkeet(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt);
   
	bool SliderFloatSkeet(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);

    bool ButtonExSkeet(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags = 0);

    bool ButtonSkeet(const char* label, const ImVec2& size_arg);
    ...
    bool ListBoxSkeet(int fix, const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items);

    bool ListBoxSkeet(int fix, const char* szLabel, int* iCurrentItem, std::function<const char* (int)> pLambda, int nItemsCount, int iHeightInItems);

	bool BeginChildSkeet(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0);

    void EndSkeet();

    void EndChildSkeet();

	void PushColor(ImGuiCol idx, ImGuiCol idx2, const ImVec4& col);

	bool TabButtonExSkeetv2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int* selected, int num, int total);

	bool TabButtonSkeetv2(const char* label, int* selected, int num, int total, const ImVec2& size_arg);

	void TabButtonSkeet(const char* label, int* selected, int num, int total);

    bool BeginSkeet(const char* name, bool* p_open, ImGuiWindowFlags flags);

    bool KeybindSkeet(const char* str_id, int* current_key, int* key_style);

	bool ColorEdit4MultiFixed(const char* label, Color* col1, Color* col2, ImGuiColorEditFlags flags, int iz);

	bool ColorEdit4Fix(const char* label, Color* col1, ImGuiColorEditFlags flags);

	bool ColorEdit4Fixed(const char* label, Color* col1, ImGuiColorEditFlags flags);

	bool ColorEdit4Multi(const char* label, Color* col1, Color* col2, ImGuiColorEditFlags flags, int iz);

	bool ColorEdit3Multi(const char* label, Color* col1, Color* col2, ImGuiColorEditFlags flags, int iz);

	void EndPopupSkeet();

	bool BeginOT(const char* name, bool* p_open, ImGuiWindowFlags flags);

    ImGuiID GetScrollbarID(ImGuiWindow* window, ImGuiAxis axis);

    void ScrollbarOT(ImGuiAxis direction);

    void TabButtonOT(const char* label, int* selected, int num, int total);

    void EndOT();

	static bool BeginChildExOT(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);

	bool BeginChildOT(const char* str_id, const ImVec2& size_arg, bool border = false, ImGuiWindowFlags flags = 0);

	void EndChildOT();

	bool SliderScalarOT(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt);

	bool SliderFloatOT(const char* label, float* v, float v_min, float v_max, const char* format, float power = 1.0f);

	bool SliderIntOT(const char* label, int* v, float v_min, float v_max, const char* format = "%.0f", float power = 1.0f);

	bool CheckboxOT(const char* label, bool* v);

	bool BeginComboOT(const char* label, const char* preview_value, ImGuiComboFlags flags, int items);

	void EndPopupOT();

	void EndComboOT();

	bool SelectableOT(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));

	bool ComboOT(const char* label, int* current_item, std::vector<const char*> items);

	void TabButton_littOT(const char* label, const char* label2, int* selected, int num, int total, ImFont* whatever);

	bool BeginNE(const char* name, bool* p_open, ImGuiWindowFlags flags);
	...
	bool ListBoxSkeet(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items);
